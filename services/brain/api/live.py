...
from api.spikes import gapped, packed
from game.players import candidates
from sim.watch import LIVE_MS

HELLO = "who are you"


def live(app: FastAPI, flies, key: str) -> None:
    # The fly thinking, as it thinks: every few milliseconds of its brain time goes out
    # as the neurons that fired in it, and the move follows when the last frame is done.
    @app.websocket("/watch")
    ...
    async def think(socket: WebSocket, stable, ask: dict) -> None:
        ...
        if not after:
            ...
            return
        await socket.send_json({"boards": len(after), "neurons": fly.player.sim.n, "frame_ms": LIVE_MS})
    ...
    async def pour(socket: WebSocket, fly, after) -> tuple:
        ...
        while True:
            ...
            if kind == "end":
                return payload
            await socket.send_bytes(gapped(payload))
